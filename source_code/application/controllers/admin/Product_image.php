...
class Product_image extends MY_Controller {
	...
	public function upload_images($id=0)
	{
		...
		if($_POST && strlen($_FILES['product_image']['name']['pi_image']))
		{
	...
	public function check_featured($pi_product_id=0)
	{
		...
		if($pi_product_id)
		{
			...
			$already_featured = $this->model_product_image->find_one($params);
			if(!$already_featured)
			{
				$this->model_product_image->autonomous_featured($pi_product_id);
			}
		}
	}

	public function featured_image($pi_product_id=0, $pi_id)
	{
		global $confg;
		...
		$this->upload_images($rec['pi_product_id']);
		exit();
